...
<div class="container">
	...
	<form action="${contextPath }/member/list" id="listForm">
		<div>
			<select name="type">
				<option value="">====</option>
				<option value="U" ${pageMaker.criteria.type eq 'U' ? 'selected':'' }>회원이름</option>
				<option value="E" ${pageMaker.criteria.type eq 'E' ? 'selected':'' }>E-mail</option>
			</select>
	...
	<table class="table">
		...
		<c:forEach items="${list}" var="m">
			<tr>
				<td>${m.id }</td>
				<td><a href="${m.id}" class="get">${m.userName }</a></td>
				<td>${m.email }</td>
			</tr>
		</c:forEach>
	</table>
	<div class="pagination">
		...
		<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }"
			var="pageNum">
			<li class="page-item"><a href="${pageNum}"
				class="page-link ${pageMaker.criteria.page == pageNum ? 'on' : '' }">${pageNum}</a></li>
		</c:forEach>
